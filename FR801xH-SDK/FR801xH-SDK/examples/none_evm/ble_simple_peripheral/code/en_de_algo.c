...
/* --- 策略 1: None (直通) --- */
static bool ops_none_encrypt(Algo_Context_t *ctx, const uint8_t *in, uint32_t len, uint8_t *out)
{
    (void)ctx;
    // 不加密，直接拷贝
    memmove(out, in, len);
...
static bool ops_none_decrypt(Algo_Context_t *ctx, const uint8_t *in, uint32_t len, uint8_t *out)
{
    (void)ctx;
    memmove(out, in, len);
    return true;
...
static bool ops_aes_encrypt(Algo_Context_t *ctx, const uint8_t *in, uint32_t len, uint8_t *out)
{
...
static bool ops_aes_decrypt(Algo_Context_t *ctx, const uint8_t *in, uint32_t len, uint8_t *out)
{
    if (ctx == NULL || in == NULL || out == NULL) {
        return false;
    }
...
{
    if (block_size == 0) return data_len;
...
static void MD5_Process(MD5_CTX *ctx, const uint8_t data[64])
{
    ...
    MD5_GET_UINT32_LE(X[ 0], data,  0); MD5_GET_UINT32_LE(X[ 1], data,  4);
    MD5_GET_UINT32_LE(X[ 2], data,  8); MD5_GET_UINT32_LE(X[ 3], data, 12);
    MD5_GET_UINT32_LE(X[ 4], data, 16); MD5_GET_UINT32_LE(X[ 5], data, 20);
    MD5_GET_UINT32_LE(X[ 6], data, 24); MD5_GET_UINT32_LE(X[ 7], data, 28);
    MD5_GET_UINT32_LE(X[ 8], data, 32); MD5_GET_UINT32_LE(X[ 9], data, 36);
    MD5_GET_UINT32_LE(X[10], data, 40); MD5_GET_UINT32_LE(X[11], data, 44);
    MD5_GET_UINT32_LE(X[12], data, 48); MD5_GET_UINT32_LE(X[13], data, 52);
    MD5_GET_UINT32_LE(X[14], data, 56); MD5_GET_UINT32_LE(X[15], data, 60);
...
static void MD5_Update(MD5_CTX *ctx, const uint8_t *input, uint32_t ilen)
{
    ...
    if (ctx->count[0] < ilen) ctx->count[1]++;
...
static void MD5_Final(MD5_CTX *ctx, uint8_t output[16])
{
    ...
    MD5_PUT_UINT32_LE(ctx->count[1] << 3 | ctx->count[0] >> 29, ctx->buffer, 60);
