...
static const uint8_t s_default_aes_key[16] = {
    'Q','S','D','f','a','g','Q','1',
    '4','1','G','S','6','J','F','8'
};
...
#define PROTOCOL_MAX_LEN     240 /* payload 最大长度，确保总长度 fits uint8_t length 字段 */
...
static uint8_t g_protocol_last_rx_crypto[PROTOCOL_MAX_CONN] = {CRYPTO_TYPE_NONE, CRYPTO_TYPE_NONE, CRYPTO_TYPE_NONE};
...
static bool Protocol_Parse(Protocol_Handler_t* self)
{
    ...
    // Footer 位于最后两个字节
    if (self->rx_buffer[self->rx_len - 2] != 0xAA || self->rx_buffer[self->rx_len - 1] != 0xAA) {
    ...
    if (pHead->length != self->rx_len) {
        co_printf("Protocol: Len mismatch %d != %d\r\n", pHead->length, self->rx_len);
        return false;
    }
     ...
     */
    self->header_info.cmd = (uint16_t)((self->header_info.cmd >> 8) | (self->header_info.cmd << 8));
    ...
    if (self->payload_len > 0) {
        ...
        if (Algo_Bind(&algo_ctx, (algo_type_t)pHead->crypto)) {
            ...
            if (!Algo_Decrypt(&algo_ctx, self->payload, self->payload_len, self->payload)) {
...
void Protocol_Handle_Data(uint8_t conidx, uint8_t* data, uint16_t len)
{
    ...
    if (conidx < PROTOCOL_MAX_CONN) {
		...
		g_protocol_last_rx_crypto[conidx] = g_protocol_handler.header_info.crypto;
    }

#if PROTOCOL_USE_ACK
    /* 如果是 ACK，匹配流水号后停止对应定时器 */
    if (cmd == CMD_ACK_ID) {
        if (conidx < PROTOCOL_MAX_CONN && g_tx_ctx[conidx].in_flight && g_tx_ctx[conidx].seq == seq) {
    ...
    if (cmd_type == 0xFE) {
        Protocol_Process_FE(cmd, g_protocol_handler.payload, g_protocol_handler.payload_len);
    }
    else if (cmd_type == 0xFD) {
        Protocol_Process_FD(cmd, g_protocol_handler.payload, g_protocol_handler.payload_len);
    }
    else if (cmd_type == 0x02) {
        /* 兼容：设备主动推送类命令（如 0x64FD/0x66FD）对应的 APP 回复（0x6402/0x6602） */
        ParamSync_OnAppReply(conidx, cmd, g_protocol_handler.payload, g_protocol_handler.payload_len);
    }
    else {
...
{
    if (conidx >= PROTOCOL_MAX_CONN) return;
...
void Protocol_Auth_SendResult(uint8_t conidx, bool ok)
{
    ...
    uint16_t enc_len = 0;
    if (!proto_encrypt_payload(crypto, plain_payload, plain_len, enc_payload, (uint16_t)sizeof(enc_payload), &enc_len)) {
        co_printf("Protocol: auth result encrypt fail\r\n");
        return;
    }

#if PROTOCOL_DEBUG_TX
    co_printf("Protocol: AuthResult enc payload (%dB crypto=0x%02X): ", (int)enc_len, (unsigned)crypto);
    ...
    co_printf("Protocol: AuthResult(0x0101) build ok=%d conidx=%d seq=%d frame_len=%d\r\n",
              ok ? 1 : 0,
              (int)conidx,
              (int)tx_seq,
              (int)frame_len);
...
{
    if (conidx >= PROTOCOL_MAX_CONN) return SP_IDX_CHAR1_VALUE;
    ...
    if (prefer == SP_IDX_CHAR1_VALUE) {
        if (sp_is_char1_ntf_enabled(conidx)) return SP_IDX_CHAR1_VALUE;
        if (sp_is_char2_ntf_enabled(conidx)) return SP_IDX_CHAR2_VALUE;
...
static bool proto_send_frame(uint8_t conidx, const uint8_t *frame, uint16_t len)
{
    if (conidx >= PROTOCOL_MAX_CONN) return false;
    ...
    co_printf("Protocol: TX pick att_idx=%d prefer=%d c1=%d c2=%d len=%d cmd=0x%04X\r\n",
              (int)att_idx,
...
{
    if (conidx >= PROTOCOL_MAX_CONN) return -1;
    if (gap_get_connect_status(conidx) == 0) return -2;
    if (len + 10 > PROTOCOL_MAX_LEN + 10) return -3;
    ...
    if (!proto_encrypt_payload(crypto, payload, len, enc_payload, (uint16_t)PROTOCOL_MAX_LEN, &enc_len)) {
		return -5;
	}

#if PROTOCOL_DEBUG_TX
    co_printf("Protocol: Unicast enc payload (%dB crypto=0x%02X): ", (int)enc_len, (unsigned)crypto);
    for (uint16_t i = 0; i < enc_len; i++) {
        co_printf("%02X ", enc_payload[i]);
    }
    co_printf("\r\n");
    ...
    frame[7 + enc_len] = bcc;
    frame[8 + enc_len] = 0xAA; frame[9 + enc_len] = 0xAA;
...
static void proto_send_ack(uint8_t conidx, uint8_t seq)
{
    uint8_t ack[10];
    uint8_t bcc = 0;
    ack[0] = 0x55; ack[1] = 0x55;
    ack[2] = 10;        // 总长度
    ack[3] = CRYPTO_TYPE_NONE;
    ack[4] = seq;
    ack[5] = 0x00; ack[6] = 0x00; // CMD_ACK_ID（大端/小端一致）
    for (uint8_t i = 0; i < 7; i++) bcc ^= ack[i];
    ack[7] = bcc;
    ack[8] = 0xAA; ack[9] = 0xAA;
...
{
    if (len + 10 > PROTOCOL_MAX_LEN + 10) return -2; // 长度超限
    ...
    for (uint8_t idx = 0; idx < PROTOCOL_MAX_CONN; idx++) {
        if (gap_get_connect_status(idx) == 0) continue;
        ...
        frame[5] = (uint8_t)(cmd >> 8);
        frame[6] = (uint8_t)(cmd & 0xFF);
        if (enc_len > 0) memcpy(&frame[7], enc_payload, enc_len);
        for (uint16_t i = 0; i < (uint16_t)(7u + enc_len); i++) bcc ^= frame[i];
        frame[7 + enc_len] = bcc;
        frame[8 + enc_len] = 0xAA; frame[9 + enc_len] = 0xAA;
