...
typedef struct {
     ...
     * @brief 加密函数接口
     */
    bool (*encrypt)(struct Algo_Context_s* ctx,
                    const uint8_t*         in,
                    uint32_t               len,
                    uint8_t*               out);
     ...
     * @brief 解密函数接口
     */
    bool (*decrypt)(struct Algo_Context_s* ctx,
                    const uint8_t*         in,
                    uint32_t               len,
                    uint8_t*               out);
} Algo_Ops_t;
 ...
 * @brief 执行加密 (内联封装)
 */
static inline bool Algo_Encrypt(Algo_Context_t* ctx,
                                const uint8_t*  in,
                                uint32_t        len,
                                uint8_t*        out) {
 ...
 * @brief 执行解密 (内联封装)
 */
static inline bool Algo_Decrypt(Algo_Context_t* ctx,
                                const uint8_t*  in,
                                uint32_t        len,
                                uint8_t*        out) {
