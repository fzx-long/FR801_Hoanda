...
#define ROLE_AS_MASTER_NUM  4

uint8_t role_m_buf[4]={0xff,0xff,0xff,0xff};
uint8_t test_buf[3]={0xAA,0xBB};
uint8_t test_num=0;
/*
 * MACROS (宏定义)
 */
...
static void app_gap_evt_cb(gap_event_t *p_event)	//GAP callback function, p_evnet is pointer to content of evnet
{
    ...
    switch(p_event->type)		//event type from BLE stack
    {
        ...
        {
            uint8_t scan_name[] = "Simpll Peripheral";
        ...
        {
            for(i=0;i<ROLE_AS_MASTER_NUM;i++)
            {
                if(role_m_buf[i]==0xff)
                {
                    role_m_buf[i]=p_event->param.master_connect.conidx;
                    if(i==(ROLE_AS_MASTER_NUM-1))
                    {
                        co_printf("gap_stop_scan \r\n");
                        gap_stop_scan();
                    }
                    else
                    {
        ...
        case GAP_EVT_DISCONNECT:	//link is disconnect evnet
        {
            ...
            for(i=0;i<ROLE_AS_MASTER_NUM;i++)
            {
                if(role_m_buf[i]==p_event->param.disconnect.conidx)                    
                {
                    role_m_buf[i]=0xff;
                    simple_central_start_scan();
                    break;
...
static uint16_t simple_central_msg_handler(gatt_msg_t *p_msg)		//GATT event callback function, p_msg is pointer to event content
{
    switch(p_msg->msg_evt)	//event type, GATT event type is defined in gatt_msg_evt_t in "gatt_api.h"
    {
        ...
        {
            //if(p_msg->att_idx == 5)	//ntf data is for UUID array idx 0 attribution
            {
...
void test_write_cmd(uint8_t *buf,uint8_t len)
{
    uint8_t i;
    buf[2]=test_num;
    gatt_client_write_t write;
    ...
    write.client_id = client_id;					//profile id assigned by BLE stack when profile is created
    write.att_idx = 1;										//This operation is for UUID array idx 1 attribution
    write.p_data = buf;	        //Buffer to be written
    write.data_len = len;										//buffer len
    
    for(i=0;i<ROLE_AS_MASTER_NUM;i++)
    {
        if(role_m_buf[i]!=0xff)
        {
            write.conidx = role_m_buf[i];
            gatt_client_write_cmd(write);
            //co_printf(" write_cmd:%d \r\n",role_m_buf[i]);
        }
        if(i==ROLE_AS_MASTER_NUM-1)
        {
            test_num++;
