...
static void hid_ntf_input_report(uint8_t con_idx, const uint8_t *data, uint16_t len)
{
    if (hid_svc_id == 0) return;
    if (con_idx >= SP_MAX_CONN_NUM) return;
    if (hid_in_ntf_enable[con_idx] == 0) return;
...
const gatt_attribute_t simple_profile_att_table[SP_IDX_NB] =
{
    ...
    [SP_IDX_SERVICE]                        =   {
                                                    { UUID_SIZE_2, UUID16_ARR(GATT_PRIMARY_SERVICE_UUID) },     /* UUID: Primary Service (0x2800) */
...
{
    if (con_idx >= SP_MAX_CONN_NUM) return false;
 ...
 */				
					if (p_msg->att_idx == SP_IDX_CHAR1_VALUE)
					{
                        ...
                        co_printf("  [rx] att_idx=%d len=%d\r\n", p_msg->att_idx, p_msg->param.msg.msg_len);
                        ...
                        Protocol_Handle_Data(p_msg->conn_idx, p_msg->param.msg.p_msg_data, p_msg->param.msg.msg_len);
					}
					else if (p_msg->att_idx == SP_IDX_CHAR2_VALUE)
					{
					...
					else if (p_msg->att_idx == SP_IDX_CHAR1_CFG)
					{
						...
						if (p_msg->conn_idx < SP_MAX_CONN_NUM) {
							ntf_char1_enable[p_msg->conn_idx] = (p_msg->param.msg.p_msg_data[0] & 0x1) ? 1 : 0;
						}
					}
                    else if (p_msg->att_idx == SP_IDX_CHAR2_CFG)
                    {
                        ...
                        show_reg(p_msg->param.msg.p_msg_data,p_msg->param.msg.msg_len,1);
                        if (p_msg->conn_idx < SP_MAX_CONN_NUM) {
                            ntf_char2_enable[p_msg->conn_idx] = (p_msg->param.msg.p_msg_data[0] & 0x1) ? 1 : 0;
                        }
                    }
				}
            break;
    ...
    switch (p_msg->msg_evt)
    {
        case GATTC_MSG_READ_REQ:
        {
            ...
            else if (p_msg->att_idx == HID_IDX_REPORT_IN_VAL)
            {
                ...
                uint16_t copy_len = (p_msg->param.msg.msg_len < sizeof(hid_input_report)) ? p_msg->param.msg.msg_len : sizeof(hid_input_report);
                memcpy(p_msg->param.msg.p_msg_data, hid_input_report, copy_len);
                return copy_len;
            }
            else if (p_msg->att_idx == HID_IDX_REPORT_OUT_VAL)
            {
                uint16_t copy_len = (p_msg->param.msg.msg_len < sizeof(hid_output_report)) ? p_msg->param.msg.msg_len : sizeof(hid_output_report);
        ...
        case GATTC_MSG_WRITE_REQ:
        {
            ...
            else if (p_msg->att_idx == HID_IDX_REPORT_IN_CFG)
            {
                if (p_msg->conn_idx < SP_MAX_CONN_NUM)
                {
                    hid_in_ntf_enable[p_msg->conn_idx] = (p_msg->param.msg.p_msg_data[0] & 0x1) ? 1 : 0;
                    co_printf("hid_in_ntf_enable[%d]=%d\r\n", p_msg->conn_idx, hid_in_ntf_enable[p_msg->conn_idx]);
                }
            }
            else if (p_msg->att_idx == HID_IDX_REPORT_OUT_VAL)
            {
