...
def SocMcu_Frame_Build(sync, feature, cmd_id, data=None):
    ...
    if SOC_MCU_USE_CRC16:
        # CRC覆盖范围：feature(2)+id(2)+len(2)+data(n)
        crc_data = frame[2:8+data_len]
        crc = crc16_ccitt_false(crc_data)
        put_u16_be(frame, crc_offset, crc)
    else:
        # CRC8（预留）
        frame[crc_offset] = 0x00
...
def SocMcu_Frame_Parse(frame):
    ...
    try:
        ...
        if SOC_MCU_USE_CRC16:
            ...
            actual_crc = crc16_ccitt_false(crc_data)
