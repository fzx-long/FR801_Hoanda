...
static void handle_uart_frame(const uart_frame_msg_t *m)
{
    ...
    if (len_field < 2u) {
        co_printf("[UART] bad len_field: %u\r\n", (unsigned)len_field);
        return;
    }
    uint16_t data_len = (uint16_t)(len_field - 2u);
    ...
    uint16_t crc_rx = (uint16_t)((crc_ptr[0] << 8) | crc_ptr[1]); /* RX CRC is BE */
    /* CRC 覆盖范围：feature(2)+id(2)+len(2)+data(n) */
    uint16_t crc_calc = 0;
    {
        uint16_t cap = (uint16_t)(2u + 2u + 2u + data_len);
        if (cap <= 256u) {
            /*
             * [稳定性] 降低栈占用：
             * - UART 帧校验路径里需要 256B 临时区；全功能测试时调用链较深，容易爆栈。
             * - 改为静态缓冲，避免栈溢出导致 HardFault 重启。
             */
            ...
            tmp[5] = f[7];
            if (data_len > 0) {
                memcpy(&tmp[6], payload, data_len);
            }
            crc_calc = crc16_ccitt_false(tmp, cap);
        } else {
            crc_ok = false;
        }
    }
    if (!crc_ok || crc_calc != crc_rx) {
        crc_ok = false;
    }
#else
    (void)crc_ptr;
    crc_ok = true; /* CRC8 未定稿时先不做校验 */
#endif
...
static int user_task_func(os_event_t *param)
{
    switch(param->event_id)
    {
        case USER_EVT_AT_COMMAND:
            {
                /* os_msg_malloc() 返回的 payload 即为字符串 */
